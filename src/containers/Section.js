...
import { toggleTaskLatency, setSearchQuery, clearLatentTasks } from '../actions/uiStateActions'
...
const mapDispatchToProps = (dispatch, { section }) => {
  return {
    ...
    onSectionComplete: (sectionId, sectionType, isSectionComplete) => {
      if (sectionType === sectionTypes.PROJECT) {
        dispatch(completeProject(sectionId, !isSectionComplete))
      }
    },
    ...
    onTaskClick: taskId => {
      browserHistory.push(`/${section}/${taskId}`)
    },
    onTaskCheckboxClick: (taskId, status) => {
      ...
      if (status) {
        dispatch(stopTaskTracking(taskId))
      }
    },
    onTaskTodayClick: (taskId, status) => {
      if (section === sectionTypes.TODAY) {
        dispatch(toggleTaskLatency(taskId, !status))
      }
      if (section === sectionTypes.INBOX) {
        dispatch(toggleTaskLatency(taskId, status))
      }
      if (section === sectionTypes.SOMEDAY) {
        dispatch(toggleTaskLatency(taskId, status))
      }
      dispatch(setTaskToday(taskId, status))
    },
    onTaskPriorityClick: (taskId, taskPriority) => {
      dispatch(editTask(taskId, { priority: taskPriority }))
    },
    ...
    addTaskContext: (taskId, contextId) => {
      dispatch(addTaskContext(taskId, contextId))
    },
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
  ...
  return {
    ...
    ...stateProps,
    ...rest,
    onSectionNameChange: newSectionName =>
      onSectionNameChange(sectionId, sectionType, newSectionName),
