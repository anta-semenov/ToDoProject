...
const order = (state = fromJS({}), action) => {
  switch (action.type) {
    ...
    case actionTypes.COMPLETE_PROJECT:
      if (action.status) {
        return state.updateIn(['project'], List(), orderArray => deleteId(orderArray, action.id))
      } else {
        return state.updateIn(['project'], List(), orderArray => addId(orderArray, action.id))
      }
    case actionTypes.REMOVE_PROJECT:
      return state.updateIn(['project'], List(), orderArray => deleteId(orderArray, action.id))
    case actionTypes.ADD_PROJECT:
      return state.updateIn(['project'], List(), orderArray => addId(orderArray, action.properties.id))
    case actionTypes.CHANGE_PROJECT_POSITION:
      return state.updateIn(['project'], List(), orderArray => changeOrder(orderArray, action.id, action.nextId))

    //contexts
    case actionTypes.DELETE_CONTEXT:
      if (action.status) {
        return state.updateIn(['context'], List(), orderArray => deleteId(orderArray, action.id))
      } else {
        return state.updateIn(['context'], List(), orderArray => addId(orderArray, action.id))
      }
    case actionTypes.REMOVE_CONTEXT:
      return state.updateIn(['context'], List(), orderArray => deleteId(orderArray, action.id))
    case actionTypes.ADD_CONTEXT:
      return state.updateIn(['context'], List(), orderArray => addId(orderArray, action.properties.id))
    case actionTypes.CHANGE_CONTEXT_POSITION:
      return state.updateIn(['context'], List(), orderArray => changeOrder(orderArray, action.id, action.nextId))

    //common
    case actionTypes.REPLACE_ORDER:
      return state.set(action.orderType, List(action.value))
    case actionTypes.SET_STATE:
      return action.state.has('order') ? action.state.get('order', List()) : state
...
export const changeOrder = (orderArray = fromJS([]), changingId, newNextId) => {
  const tempArray = orderArray.filter(item => item !== changingId && item !== undefined)
