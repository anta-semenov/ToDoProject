...
describe('Task action creators', () => {
  test('Should create an action to add a task', () => {
    ...
    const action = actions.addTask(properties)
    expect(action).toMatchObject({ type: types.ADD_TASK, properties })
  ...
  test('Should create an action to complete a task', () => {
    ...
    expect(action).toMatchObject({ type: types.COMPLETE_TASK, id, status })
    expect(action.date).toBeLessThanOrEqual(Date.now())
  })
