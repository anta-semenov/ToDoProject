...
describe('Project action creators', () => {
    test('Should create an action to add a project', () => {
      ...
      expect(actions.addProject(properties)).toEqual(
          {type: types.ADD_PROJECT, properties: { ...properties, createdDate: Date.now() }}
      );
    })
    ...
    test('Should create an action to complete a project', () => {
        const id = 0
        const status = true
        expect(actions.completeProject(id, status)).toEqual({ type: types.COMPLETE_PROJECT, date: Date.now(), id, status })
    })
